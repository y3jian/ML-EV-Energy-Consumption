...
results = []
output_dir = _PKG / "results" / "random_forest"
...
for name, loader in datasets.items():
    ...
    try:
        ...
        plot_actual_vs_pred(
            y_test,
            y_pred,
            f"Random Forest - {name} (Actual vs Predicted)",
            output_dir / f"{ds}_actual_vs_pred.png",
        )
        plot_residuals(
            y_test,
            y_pred,
            f"Random Forest - {name} (Residuals)",
...
print("\n=== Random Forest (optimize driving efficiency) ===")
print(results_df)

out_path = Path("optimize_efficiency_rf_results_comparison.csv")
...
if not results_df.empty:
    plot_bar(
        results_df,
        "Test_RMSE",
        "Random Forest - test RMSE by dataset",
        output_dir / "comparison_rmse.png",
    )
    plot_bar(
        results_df,
        "Test_R2",
        "Random Forest - test R2 by dataset",
        output_dir / "comparison_r2.png",
    )
    plot_bar(
        results_df,
        "Total_s",
        "Random Forest - total runtime by dataset",
